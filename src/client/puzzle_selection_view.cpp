...
PuzzleSelectionView::render_game()
{
  ...
  LOG_D("view") << "Model has hovered ?" << dep_model_.has_hovered() << std::endl;
  ...
  if (dep_model_.has_hovered()) {
    dep_renderer_.highlight_puzzle_name(dep_model_.get_hovered_index());
  }
...
PuzzleSelectionView::handle_event(int i_event)
{
  if (i_event == GameEvent::MOUSE_CLICKED) {
    ...
    if (puzzle_index != -1 && puzzle_index < dep_model_.get_puzzle_count()) {
      dep_model_.set_selected_index(puzzle_index);
    } else if (puzzle_index == -1 
      ...
      dep_model_.set_selected_index(0);
	       ...
	       && dep_model_.has_hovered()
	       && dep_model_.get_hovered_index() == dep_model_.get_puzzle_count() - 1) {
      dep_model_.set_selected_index(dep_model_.get_puzzle_count() - 1);
    }
